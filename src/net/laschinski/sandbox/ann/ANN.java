...
public class ANN {
	...
	public void AddLayer(int numberOfNeurons, String activationFunction) {
		...
		// on all the not-first layers we use the number of outputs of the previous
		// layer
		else {
	...
	public ArrayList<Double> Predict(ArrayList<Double> inputValues) {
		...
		for (int i = 0; i < layers.size(); i++) {
			...
			// on all the not-first layers we use the outputs of the previous layer as
			// inputs
			else {
				// uses the outputs of the previous iteration as inputs
				inputs = new ArrayList<>(outputs);
				// since this is at least the second iteration, we need to clean up the outputs
				// before calculating them again
				outputs.clear();
			}
	...
	public void TrainBatch(ArrayList<ArrayList<Double>> inputValues, ArrayList<ArrayList<Double>> desiredOutputValues) {
		...
		for (int i = 0; i < inputValues.size(); i++) {
			// after the first iteration the deltas are cleanly overwritten, so we can start
			// to sum up now
			if (i > 0) {
				sumDeltas = true;
			}
			...
			for (int j = 0; j < layers.get(i).neurons.size(); j++) {
				...
				// neuron is at any not-output layer
					...
					for (int p = 0; p < layers.get(i + 1).neurons.size(); p++) {
						// sum up the error gradients of -every neuron at the following layer-
						// multiplied by -the weight for this neuron-
						errorGradSum += layers.get(i + 1).neurons.get(p).getErrorGradient()
								* layers.get(i + 1).neurons.get(p).getWeight(j);
					}
					// caluclate the error gradient with regards to the error gradients of the
					// connected neurons
			...
			for (int j = 0; j < layers.get(i).neurons.size(); j++) {
				...
				for (int k = 0; k < layers.get(i).neurons.get(j).getWeightSize(); k++) {
					...
					// neuron is not at the output layer
						...
						double delta = alpha * layers.get(i).neurons.get(j).getErrorGradient()
								* layers.get(i).neurons.get(j).getInput(k);

						deltas.add(delta);
					}
				}

				// delta for the bias is always calculated by the gradient saved in the neuron
				// object
	 ...
	 * 
	 * /// <summary> /// Convertes the ANN weights to an XML document which can be
	 * saved for later restore. /// </summary> /// <returns>An XDocument containing
	 * neuron weight values</returns> private XDocument brainToXml() { XElement
	 * brainXml = new XElement("Brain"); brainXml.Add(new XElement("Inputs",
	 * numInputs)); XDocument brainDoc = new XDocument(new XDeclaration("1.0",
	 * "UTF-16", null), brainXml);
	 * 
	 * for (int i = 0; i < layers.Count; i++) { XElement layerXml = new
	 * XElement("Layer"); layerXml.Add(new XElement("LayerID", i + 1));
	 * brainXml.Add(layerXml);
	 * 
	 * for (int j = 0; j < layers[i].neurons.Count; j++) { XElement neuronXml = new
	 * XElement("Neuron"); layerXml.Add(neuronXml);
	 * 
	 * List<double> weights = layers[i].neurons[j].getWeights(); for (int k = 0; k <
	 * weights.Count; k++) { XElement weightXml = new XElement("Weight",
	 * weights[k]); neuronXml.Add(weightXml); } XElement biasXml = new
	 * XElement("Bias", layers[i].neurons[j].getBias()); neuronXml.Add(biasXml); } }
	 * 
	 * return brainDoc; }
	 * 
	 * /// <summary> /// Restores the ANN weights from an XML document. ///
	 * </summary> /// <param name="brainDoc">An XDocument containing neuron weight
	 * values</param> private void XmlToBrain(XDocument brainDoc) { XElement
	 * brainXml = brainDoc.Element("Brain"); IEnumerable<XElement> layersEnum =
	 * brainXml.Elements("Layer"); int i = 0; foreach (XElement layerElement in
	 * layersEnum) { IEnumerable<XElement> neuronsEnum =
	 * layerElement.Elements("Neuron"); int j = 0; foreach (XElement neuronElement
	 * in neuronsEnum) { IEnumerable<XElement> weightsEnum =
	 * neuronElement.Elements("Weight"); List<double> newWeights = new
	 * List<double>(); foreach (XElement weightElement in weightsEnum) {
	 ...
	 * )); j++; } i++; } }
	 * 
	 * /// <summary> /// Saves a saved brain to a file (Work in progress) ///
	 * </summary> public void saveBrain() { brainToXml().Save("D:\\savedBrain.xml");
	 * }
	 * 
	 * /// <summary> /// Loads a saved brain from a file (Work in progress) ///
	 * </summary> public void loadBrain() { XDocument xdocument =
	 * XDocument.Load("D:\\loadBrain.xml"); XmlToBrain(xdocument); }
	 */
}
